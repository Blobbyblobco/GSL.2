...
// Weekly Lotto games: how many numbers each draw has, and the jackpot shown by default.
export const LOTTO_GAMES = Object.freeze({
  mega7: { name: 'Mega 7', picks: 7, jackpot: '$1,000,000' },
  wild5: { name: 'Wild 5', picks: 5, jackpot: '$250,000' },
  fast5: { name: 'Fast 5', picks: 5, jackpot: '$100,000' },
  easy6: { name: 'Easy 6', picks: 6, jackpot: '$50,000' },
});

export const DEFAULT_SETTINGS = Object.freeze({
  // nextDraw is free text (e.g. "Sat 4 Oct, 8pm"); empty means the site just says "Weekly draw".
...
function validateLottoGame(id, g, errors) {
  const { name, picks } = LOTTO_GAMES[id];
  ...
  for (const [field, v] of Object.entries(g)) {
    if (field === 'jackpot') {
      ...
      v.forEach((r, i) => {
        ...
        if (!Array.isArray(nums) || nums.length !== picks || !nums.every((n) => Number.isInteger(n) && n >= 0 && n <= 99)) {
          errors.push(`${where}: needs ${picks} whole numbers from 0 to 99.`); return;
        }
