...
// The handler takes its storage as an argument so it can be tested without Vercel or Redis.
...
export const DEFAULT_SETTINGS = Object.freeze({
  // nextDraw is free text (e.g. "Sun 5 Oct, 3pm"); empty means the site shows the game's regular draw day.
...
export const STORAGE_MISSING = 'Storage is not set up. Connect an Upstash Redis database to the project (Vercel → Storage).';

/**
 * Returns an error Response if the request isn't from the admin, or null if it is.
 * failDelayMs: pause after a wrong password to slow down guessing.
 */
export async function checkAdmin(req, adminPassword, failDelayMs = 800) {
  if (!adminPassword || adminPassword.length < 12) {
    return json({ error: 'Admin is not set up. Set an ADMIN_PASSWORD (12+ characters) in the project environment variables.' }, 503, NO_STORE);
  }
...
/**
 * store: { get(key) -> object|null, set(key, object) }, or null when storage isn't configured
 * (the site then shows the defaults and saving is refused).
 */
export async function handleSettingsRequest(req, { store, adminPassword, failDelayMs = 800 }) {
  ...
  if (req.method === 'GET' || req.method === 'HEAD') {
    const settings = withDefaults(store ? await store.get(SETTINGS_KEY) : null);
